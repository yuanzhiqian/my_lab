...
from notifier.dingding_notifier import *

class Monitor(object):
    def __init__(self, script = "./test_online.sh", interval = 5, messenger = None):
        self.script = script;
    ...
    def trigger_notice(self, reason):
        if self.messenger == None:
            ...
            # use dingding for now
            time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()));
            err_rate = float(self.total_err) / float(self.total_req);
            self.messenger.notify("[" + time_str + "]" + " Error occurs: " + reason + "\n"
                                  + "Fail rate: " + str(err_rate * 100) + "%");
...
if __name__ == '__main__':
    ...
    monitor = Monitor(interval = 1, messenger = dingding);
    monitor.start();
